...
const AdminDashboard = () => {
  ...
  // UI state
  const [dashboardStatistics] = useState(stats);

  // Data states (initialized with mock data for display)
  const [bookingCount, _setBookingCount] = useState([
    { name: "Summer Fest", y: 45 },
    { name: "Tech Talk", y: 20 },
    { name: "Art Expo", y: 35 }
  ]);
  const [bookingCapacity, _setBookingCapacity] = useState([
    { name: "Summer Fest", y: 50 },
    { name: "Tech Talk", y: 30 },
    { name: "Art Expo", y: 40 }
  ]);
  const [cancelCount, _setCancelledCount] = useState([
    { name: "Summer Fest", y: 2 },
    { name: "Tech Talk", y: 1 },
    { name: "Art Expo", y: 0 }
  ]);
  const [bookingRevenue, _setBookingRevenue] = useState([
  ...
  const loadEvents = () => {
    ...
    // TODO: Integrate API here to fetch real data
    // Example of how setters would be used:
    // _setBookingCount(fetchedBookingData);
    // _setBookingCapacity(fetchedCapacityData);
    // _setCancelledCount(fetchedCancelData);
    // _setBookingRevenue(fetchedRevenueData);
    // _setEventStatus(fetchedEventStatusData);
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl shadow-sm p-5 border border-gray-100">
          ...
          {bookingCount.length === 0 ? (
            <p className="flex items-center justify-center min-h-75 text-gray-400">No Data Available</p>
          ) : (
